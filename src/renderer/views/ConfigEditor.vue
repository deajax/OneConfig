<template>
    <div class="flex flex-col h-full overflow-hidden" @dragover.prevent @drop.prevent="handleDrop">
        <!-- 注入头部中区：视图切换 tabs -->
        ...
        <Teleport v-if="headerStore.rightEl" :to="headerStore.rightEl">
            <div class="flex items-center gap-1">
                <a-button :icon="h(FolderOpenOutlined)" @click="handleOpenFile"> 打开 </a-button>
                <a-button type="primary" :icon="h(SaveOutlined)" :disabled="!configStore.isDirty" @click="handleSave"> 保存 </a-button>
                <a-dropdown>
        ...
        <!-- 主体：可视化 -->
        <div v-show="activeView === 'visual'" class="flex-1 overflow-hidden flex flex-col bg-white">
        ...
        <!-- 底栏 -->
        <footer class="flex items-center justify-between px-6 h-7 shrink-0 border-t border-gray-100 bg-white">
...
onMounted(() => {
    headerStore.setBack(() => handleBack());
    headerStore.setTitle(displayName.value);
});
watch(displayName, (v) => headerStore.setTitle(v));
onUnmounted(() => {
