		...
		<div class="provider-list">
			...
			<a-row v-else :gutter="[16, 16]">
				<a-col v-for="profile in providerStore.profiles" :key="profile.id" span="8">
					<a-card hoverable @click="handleEdit(profile)">
						<a-card-meta
							:description="profile.envVars['ANTHROPIC_BASE_URL'] || '空'"
						>
							<template #avatar>
								<img
									v-if="profile.icon && isImageIcon(profile.icon)"
									:src="getIconSrc(profile.icon)"
								...
								<div class="flex items-center">
									...
									<span @click.stop>
										<a-switch
											...
											@change="handleActivate(profile.id)"
										/>
									</span>
								</div>
							</template>
						...
						<template #actions>
							<div class="" @click.stop>
								<a-select
									...
									:value="profile.envVars['ANTHROPIC_MODEL'] || undefined"
									placeholder="选择模型…"
									class="w-full"
									show-search
		...
		<div class="flex flex-col gap-4">
			...
			<div class="flex gap-3">
				...
				<div class="flex flex-col items-center gap-1">
					<div
						class="w-[40px] h-[40px] border border-dashed border-gray-300 rounded flex items-center justify-center cursor-pointer hover:border-blue-400 transition-colors overflow-hidden bg-white"
						@click="triggerIconUpload"
					>
						<img
							v-if="modalForm.icon && isImageIcon(modalForm.icon)"
							:src="getIconSrc(modalForm.icon)"
					...
					<span class="text-[10px] text-gray-400">点击上传</span>
				</div>
				<input
					ref="iconInputRef"
					type="file"
					class="hidden"
					accept=".jpg,.jpeg,.png,.svg,.gif,.webp,.ico"
...
<script setup lang="ts">
	...
	function isImageIcon(icon: string): boolean {
		return /\.(jpe?g|png|svg|gif|webp|ico)$/i.test(icon);
	}
	...
	async function handleIconUpload(e: Event) {
		...
		if (!file) return;
		// 读取为 base64 并通过 IPC 保存
		const reader = new FileReader();
		reader.onload = async () => {
			const dataUrl = reader.result as string;
			const res = await window.electronAPI.uploadProviderIcon(file.name, dataUrl);
			if (res.success) {
				modalForm.icon = res.fileName;
			}
		};
