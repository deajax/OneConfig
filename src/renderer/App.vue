<template>
	...
	>
		<a-layout class="h-screen overflow-hidden">
			<a-layout-header
				class="px-2! h-12! leading-12! bg-transparent!"
			...
			>
				<header class="relative flex items-center h-full">
					...
					<!-- 收起/展开按钮 -->
					<div
						:class="isWin ? 'pl-2' : 'ml-20'"
						style="-webkit-app-region: no-drag"
					>
						<a-button
							type="text"
							:icon="collapsed ? h(LayoutOutlined) : h(LayoutTwoTone)"
							@click="collapsed = !collapsed"
						/>
					</div>
					<div
						v-if="!isWin"
						class="absolute h-full top-0 bottom-0 left-1/2 -translate-x-1/2"
					>
						<h3 class="">OneConfig</h3>
					</div>
					<!-- 右侧操作区 -->
					<a-space
						class="flex items-center gap-1 ml-auto mr-0"
						style="-webkit-app-region: no-drag"
					>
						<a-button
							type="text"
							:icon="
								h(QuestionCircleOutlined, { style: 'font-size: 18px;' })
							"
							@click="openHelp"
						/>
						<a-button
						...
						<div class="ml-2" v-if="isWin">
							...
							<a-button
								type="text"
								class="win-control-btn win-close-btn"
								@click="closeWindow"
							>
								<template #icon>
									<CloseOutlined />
		...
		<a-drawer
			v-model:open="helpDrawer"
			title="Basic Drawer"
			placement="right"
			:close-icon="h('span', { class: 'drawer-close-btn' }, [h(CloseOutlined)])"
			wrap-class-name="help-drawer-wrap"
		>
			<p>Some contents...</p>
			<p>Some contents...</p>
			<p>Some contents...</p>
...
<script setup lang="ts">
	...
	function openGithub() {
		window.open("https://github.com/deajax/OneConfig", "_blank");
	}
...
<style lang="less">
	// Drawer 整体不受拖拽区域影响（drawer 在 portal 中，scoped 样式无法到达）
	.help-drawer-wrap {
